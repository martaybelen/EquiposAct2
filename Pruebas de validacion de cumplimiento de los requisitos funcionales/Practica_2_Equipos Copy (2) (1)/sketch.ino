...
void loop() {
 ...
 if (!btnState && flag && millis()-timerBoton >50){
  ...
  if(modo == 2){ //Entramos en modo 2: Iluminacon
    ...
    // Mostrar el valor leído y el porcentaje calculado en el monitor serie
   ...
   // Limitar el número de LEDs a encender entre 0 y numLeds
...
int controlarIluminacion(float lux, int leds_encendidos) {
  
  if(lux<78){
    leds_encendidos = leds_encendidos +1;    
  }
  else if(lux > 82){
    leds_encendidos = leds_encendidos -1;    
  }
  
  if(leds_encendidos > 8){ leds_encendidos = 8;}
  else if (leds_encendidos < 0){leds_encendidos =0;}

  // Encender el número adecuado de LEDs
  for (int i = 0; i < leds_encendidos; i++) {
    digitalWrite(ledPins[i], HIGH);   
  }
  for (int i = leds_encendidos+1; i < NUM_LEDS; i++) {
    digitalWrite(ledPins[i], LOW);   
  }
  return leds_encendidos;
