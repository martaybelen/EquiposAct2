...
//-------Sensor LM35-------//
#define LM35PIN A1 // Pin analógico al que está conectado el LM35
...
void loop() {
  ...
  if (modo == 1) { //Entramos en modo 1: Temperatura y Humedad
    ...
    //Se muestra las medidas tomadas del sensor en el Monitor Serie:
  ...
  if (modo == 2) { //Entramos en modo 2: Iluminacon
    //En este modo se muestra la iluminación en el Monitor Serie y se controla a intensidad con 8LEDS
  ...
  if (modo == 3) { //Entramos en modo 3: Presencia
    ...
    //Se muestra la medida en el Monitor Serie
  ...
  if (modo == 4) { //Modo control de temperatura
    ...
    temp_externa_actual = (lecturaLM35 * 5.0 / 1024.0) * 100.0;

    distanciaanterior = distancia;
    distancia = medirDistancia();

    if (isnan(temp_interna_actual) || isnan(temp_externa_actual)) {
      Serial.println("Error al leer la temperatura de uno de los sensores");
      return;
    }
...
void mostrar_temperaturas() {
  //Mostramos temperaturas en el Monitor Serie
  Serial.print("Temp. Externa: ");
  Serial.print(temp_externa_actual);
  Serial.print(" C, Temp. Interna: ");
  Serial.print(temp_interna_actual);
  Serial.println(" C");
}
