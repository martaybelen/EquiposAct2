...
void loop() {
 ...
 if (!btnState && flag && millis()-timerBoton >50){
  ...
  if(modo == 2){ //Entramos en modo 2: Iluminacon
    ...
    //Mantener palanca hacia la derecha en el circuito físico para modo nocturno
    ...
    if(primera_medida==true){
      ...
      //lux_anterior = lux;
      //lux = map(ldrValue, 0, 1023, 0, 1000);
      //lux=1-lux;
  ...
  delay(200);
     ...
     //lectura del valor LDR
      lux = (1023 - ldrVal) / 1023.0 * 100; // Normaliza el valor a un porcentaje
      Serial.println(lux_anterior);
      Serial.println(lux);

      //lux = (1023 - ldrVal) / 1023.0 * 100;
      //lux =1-lux;
      //lux = map(ldrVal, 0, 1023, 0, 100);
...
if (modo == 4){//Modo control de temperatura
 ...
 // sensors.requestTemperatures();
  ...
  temp_externa_actual = analogRead(sensorPin);
  distanciaanterior = distancia;
  distancia = medirDistancia();
...
float controlarIluminacion(float lux) {
 ...
 // Control de los LEDs
    if(lux > lux_anterior+1 || lux < lux_anterior-1 ){
